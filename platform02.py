...
class Platform02(BaseExport, BaseImport, BaseSync):
    ...
    def __init__(self, databases: list[str]):
        ...
        # 要导入的目的mysql
        self.target_host = config.get('platform_mysql', 'host')
        self.target_port = config.get('platform_mysql', 'port')
        self.target_user = config.get('platform_mysql', 'user')
        self.target_pass = config.get('platform_mysql', 'pass')
